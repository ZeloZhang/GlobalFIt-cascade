...
namespace NuFit
{
	class interpolated_sys
	{
		...
		private:	
			void fill_hists();
			void create_fit(const double &baseline_val, const double &baseline_err, std::vector<TH3D *> &hists, const unsigned int &k, const unsigned int &l, const unsigned int &m, std::vector<std::vector<std::vector<bool>>> &success, std::string component);
