	...
	for (unsigned int k=0; k<dataset.get_nbinsx(); ++k) 
	{
		for (unsigned int l=0; l<dataset.get_nbinsy(); ++l) 
		{
			for (unsigned int m=0; m<dataset.get_nbinsz(); ++m) 
			{
				...
				create_fit(baseline_expectation_conv, baseline_error_conv, hists_conv, k, l ,m, success_conv, "Conv");
				create_fit(baseline_expectation_prompt, baseline_error_prompt, hists_prompt, k, l ,m, success_prompt, "Prompt");
				create_fit(baseline_expectation_astro, baseline_error_astro, hists_astro, k, l ,m, success_astro, "Astro");
...
void NuFit::interpolated_sys::create_fit(const double &baseline_val, const double &baseline_err, std::vector<TH3D *> &hists, const unsigned int &k, const unsigned int &l, const unsigned int &m, std::vector<std::vector<std::vector<bool>>> &success, std::string component)
{
